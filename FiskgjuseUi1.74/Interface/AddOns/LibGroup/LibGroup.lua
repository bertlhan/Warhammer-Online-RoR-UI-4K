...
local MAX_POINTS = 700; -- (was 300) The maximum number of objects that can be displayed on the map (worldwide). This number could change over time and exceed this value
...
local function GetScenarioData()
    ...
    for index, memberData in ipairs(scenarioData) do
        if (memberData.sgroupindex > 0) then
            ...
            table_insert(scenarioGroups[scenarioGroup], {index, slotInGroup});
        end
    end
