...
{
    start               = {x=150, y=0},   --{x=-120, y=0}
    target              = {x=150, y=-80}, --{x=-120, y=-80}
    current             = {x=150, y=0},   --{x=-120, y=0},
    maximumDisplayTime  = 4,
    fadeDelay           = 2,
    fadeDuration        = 0.75,
}
...
EA_System_EventEntry = Frame:Subclass("EventTextTemplate")
...
function EA_System_EventEntry:Create(windowName, parentWindow, animationData)
    ...
    then
       ...
       eventFrame.m_Label = windowName.."Label"
        eventFrame.m_Icon = windowName.."Icon"
    end
                                       
   
 --   WindowSetOffsetFromParent(windowName, animationData.start.x, 400)
...
function EA_System_EventEntry:SetupText(hitTargetObjectNumber, hitAmount, textType, ability)
    local text = L""
    --d(towstring(ability))
    ...
    -- added by Fox 14.06.2019
    LabelSetText( self.m_Label, text )
    LabelSetTextColor( self.m_Label, color.r, color.g, color.b )
    WindowSetFontAlpha( self.m_Label, 1.0 )

    if (ability or ability~=nil) and (ability~=0) then
        local data = GetAbilityData(ability)
        --d(L"icon: " .. towstring(data.iconNum))
        if data.iconNum ~= 0 then
        local texture, x, y = GetIconData(data.iconNum)
            DynamicImageSetTexture(self.m_Icon,texture,x,y)
            DynamicImageSetTextureScale(self.m_Icon, 0.5)
        end
    end
        ...
        then
            ...
            then
                ...
                then
                    ...
                    newEventFrame:SetupText(self.m_TargetObject, eventData.amount, eventData.type, eventData.abilityID)
...
function EA_System_EventText.AddCombatEventText( hitTargetObjectNumber, hitAmount, textType, ability)
    local eventData = { event=COMBAT_EVENT, amount = hitAmount, type = textType, abilityID = ability }
