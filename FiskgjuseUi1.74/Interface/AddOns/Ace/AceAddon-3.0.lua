--- **WAR-AceAddon-3.0** provides a template for creating addon objects.
-- It'll provide you with a set of callback functions that allow you to simplify the loading
-- process of your addon.\\
-- Callbacks provided are:\\
-- * **OnInitialize**, which is called directly after the addon is fully loaded.
-- * **OnEnable** which gets called after SystemData.LoadingData.isLoading is false, when most of the data provided by the game is already present.
-- * **OnDisable**, which is only called when your addon is manually being disabled.
-- @usage
-- -- A small (but complete) addon, that doesn't do anything, 
-- -- but shows usage of the callbacks.
-- local MyAddon = LibStub("WAR-AceAddon-3.0"):NewAddon("MyAddon")
-- 
...
-- end
local MAJOR, MINOR = "WAR-AceAddon-3.0", 6
...
function AceAddon:EnableAddon(addon)
	...
	safecall(addon.OnModulesEnabled, addon )
...
function AceAddon_OnUpdate_DONOTTOUCH()
    if (not aceLoaded) and (SystemData.LoadingData.isLoading == false) then
        AceAddon.InitEnable()
        UnregisterEventHandler(SystemData.Events.UPDATE_PROCESSED, "AceAddon_OnUpdate_DONOTTOUCH")
