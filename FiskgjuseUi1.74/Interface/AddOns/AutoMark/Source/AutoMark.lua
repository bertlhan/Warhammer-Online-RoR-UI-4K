...
local function CreateMarker (object_id, career_id)
  ...
  local career = CAREERS[career_id]
  local add_circle = true
  local archetype_name = "icon022701"

  if (career.type == "mdps")
  then
    archetype_name = "icon022657"
    WindowSetScale(window_name, 0.7)
	add_circle = false or GameData.Player.isInScenario
		if (add_circle or career_id == AutoMarkGuardId)
			then
				WindowSetScale(window_name, 0.6)
				WindowSetTintColor(window_name .. "_Border", 255, 63, 65)
		end
  
  elseif (career.type == "rdps")
  then
    archetype_name = "icon022653"
        WindowSetScale(window_name, 0.7)
	add_circle = false or GameData.Player.isInScenario
		if (add_circle or career_id == AutoMarkGuardId)
			then
				WindowSetScale(window_name, 0.6)
				WindowSetTintColor(window_name .. "_Border", 255, 173, 0)
		end
 
 elseif (career.type == "healer")
  then
    archetype_name = "icon022706"
        WindowSetScale(window_name, 0.7)
	add_circle = false or GameData.Player.isInScenario
		if (add_circle or career_id == AutoMarkGuardId)
			then
				WindowSetScale(window_name, 0.75)
				WindowSetTintColor(window_name .. "_Border", 144, 105, 209)
		end	
 
 elseif (career.type == "tank")
  then
    add_circle = true
		if (add_circle or career_id == AutoMarkGuardId)
			then
				WindowSetScale(window_name, 0.8)
				WindowSetTintColor(window_name .. "_Border", 190, 150, 95)
		end 
		if GameData.Player.isInScenario
			then
				WindowSetScale(window_name, 1)
		end
 end

  local marker = {}
  marker.object_id = object_id
  marker.career_id = career_id
  marker.window_name = window_name
  marker.career_texture_name = texture_name
  marker.archetype_texture_name = archetype_name
  marker.archetype = career.type

  DynamicImageSetTexture (window_name .. "_Border", "AutoMark_Circle", texture_x, texture_y)


  if (add_circle or career_id == AutoMarkGuardId)
	then
		WindowSetAlpha(window_name .. "_Border", 1)   
			else
				texture_name = archetype_name
				DynamicImageSetTextureDimensions (window_name .. "_Icon", 64, 64)
				WindowSetAlpha(window_name .. "_Border", 0)
  end
...
function AutoMark.OnPlayerTargetUpdated (target_type, object_id, object_type)
  ...
  -- update AutoMarkGuardId
  if( targetType == SystemData.TargetObjectType.NONE ) then
    AutoMarkGuardId = nil
  end

  --[[
  if (target_type == "selfhostiletarget" and GameData.Player.isInScenario == true)
  then

    TargetInfo:UpdateFromClient()
    local targetBuffs = GetBuffs( GameData.BuffTargetType.TARGET_HOSTILE)

    local guardId = nil
    for index, buff in pairs(targetBuffs) do
      local buffId = buff.abilityId
      if bufftId == 1674 then
        guardId = GameData.CareerLine.BLACK_ORC
      elseif buffId == 8325 then
        guardId = GameData.CareerLine.CHOSEN
      elseif buffId == 9325 then
        guardId = GameData.CareerLine.BLACKGUARD
      end
    end
    if guardId then
      AutoMarkGuardId = guardId
    else
      AutoMarkGuardId = nil
    end
    --d("AutoMarkGuardId: " .. tostring(AutoMarkGuardId))
  end
  ]]--
...
-- --   if (marker.archetype == "tank" and GameData.Player.isInScenario == true) then
-- --     if AutoMarkGuardId == marker.career_id then
-- --       DynamicImageSetTexture (marker.window_name .. "_Icon", marker.career_texture_name, 0, 0)
-- --       DynamicImageSetTextureDimensions (marker.window_name .. "_Icon", 32, 32)
--        WindowSetAlpha(marker.window_name .. "_Border", 1)
--        WindowSetScale(marker.window_name, 1.3)
-- -       WindowSetTintColor(marker.window_name .. "_Border", 255, 255, 0)
--      else
--        DynamicImageSetTextureDimensions (marker.window_name .. "_Icon", 64, 64)
--        DynamicImageSetTexture (marker.window_name .. "_Icon", marker.archetype_texture_name, 0, 0)
--        WindowSetScale(marker.window_name, 1)
--        WindowSetAlpha(marker.window_name .. "_Border", 0)
 --     end
 --   end
