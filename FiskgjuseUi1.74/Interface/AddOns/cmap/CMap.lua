...
local GetZoneName = GetZoneName
local WindowSetDimensions = WindowSetDimensions
local GetStringFormat = GetStringFormat
...
function CMapWindow.CMapGetSettings()
	...
	CMapWindow.VisSettings[BACKGROUND] = {visible=true,alpha=1,red=0,green=0,blue=0,point="topleft",rpoint="topleft",rto=WindowName,x=0,y=0,type=TYPEBGWINDOW}
	...
	CMapWindow.VisSettings[CFG_BORDER] = {value=2,descr="Number defines the bordersize",type=TYPESETTING,dtype="number"}
	CMapWindow.VisSettings[CFG_AREA] = {value=1,descr="True shows only the Zonename [1] the Areaname[2] or both [3] or off[4]",type=TYPESETTING,dtype="number"}
	...
	CMapWindow.VisSettings[CFG_INF] = {value=false,descr="Switches the InfluenceArea visibility [true] or [false]",type=TYPESETTING,dtype="boolean"}
...
function CMapWindow.Initialize()
	...
	-- city sc button fix 2020-06-27
	RegisterEventHandler( SystemData.Events.LOADING_END, "CMapWindow.updateScenarioButton" );
	RegisterEventHandler( SystemData.Events.INTERFACE_RELOADED, "CMapWindow.updateScenarioButton" );
    ...
    WindowRegisterEventHandler( "CMapWindow", SystemData.Events.MAILBOX_MESSAGE_OPENED,       		"CMapWindow.ClearMailIcon")
...
function CMapWindow.updateScenarioButton()
...
function CMapWindow.ClearMailIcon()
...
function CMapWindow.ToggleScenarioSummaryWindow()
	...
	--  or (WindowGetShowing("ScenarioSummaryWindow") == true)
...
function CMapWindow.OnMouseoverScenarioSummaryBtn()
    ...
    column = column + 1
    Tooltips.SetTooltipColor( row, column, 140, 100, 0 )
    --Tooltips.SetTooltipText( row, column, L"("..SystemData.Settings.Keybindings.TOGGLE_SCENARIO_SUMMARY_WINDOW.bindings[1]..L")" )
...
-- When the player moves to a different area, we need to update the area Text
    ...
    zw = L""..GetZoneName(MapGetPlayerLocationMaps()[2])
    end
...
function CMapWindow.RefreshMapPointFilterMenu()
    ...
    do
        table_insert(filterDisplayOrder, index)
    end
    ...
    do
        table_insert(gutterDisplayOrder, index)
    end
