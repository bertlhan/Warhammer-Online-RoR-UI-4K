...
local SetTintColor = SetTintColor
local ButtonSetTexture = ButtonSetTexture
local ButtonSetTextureSlice = ButtonSetTextureSlice
...
function TexturedButtons.Initialize() 
	...
	RegisterEventHandler(SystemData.Events.USER_SETTINGS_CHANGED, "TexturedButtons.UpdateActionbars");
			...
			if (cooldownSettings.RemoveS) then
				if (self.m_Cooldown > 3) then
					if (self.m_Cooldown >= 60) then
						cooldownTimer:SetText(wstring.format(L"%dm", math_ceil(self.m_Cooldown / 60)));
					else
