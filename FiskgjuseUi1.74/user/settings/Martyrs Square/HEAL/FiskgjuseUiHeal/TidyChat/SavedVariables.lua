TidyChat.Settings = 
{
	
	{
		scrollbar_position = 3,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	},
	
	{
		scrollbar_position = 1,
		scrollbar_fadeout_enable = true,
		scrollbar_fadein_alpha = 1,
		scrollbar_fadeout_alpha = 0.25,
		chatwindow_background_showing = false,
		chatwindow_click_through = false,
		chatwindow_tabs_showing = true,
	}, 
	textentry_channel_alpha = 1,
	textentry_anchor_point = 2,
	tobottom_showing = true,
	advanced_loot_roll_showing = false,
	textentry_background_alpha = 0,
	copy_showing = true,
	social_showing = false,
	textentry_relative_to = 1,
	textentry_free = true,
	version = 9,
	auction_filter_showing = false,
	textentry_channel_showing = false,
}



