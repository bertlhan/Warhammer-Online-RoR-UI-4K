LayoutEditor.Settings = 
{
	ClosetGoblinOptionWindow = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1136 = 
	{
		locked = false,
		hidden = false,
	},
	CDownWindow = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHT2 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window206 = 
	{
		locked = false,
		hidden = false,
	},
	HostileTarget = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window37 = 
	{
		locked = false,
		hidden = false,
	},
	PrimaryTargetLayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	Rp = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_ScenarioTracker = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_ROR_RRQTracker = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window437 = 
	{
		locked = false,
		hidden = false,
	},
	TargetsFriendly1 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHTOHP2 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_HostileBuffs = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Hostile_Morales = 
	{
		locked = false,
		hidden = false,
	},
	SwingerWindow = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Hostile_Debuffs = 
	{
		locked = false,
		hidden = false,
	},
	["DAoCBuff_Hostile Buffs"] = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar2 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiRP = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHT = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_BattlefieldObjectiveTracker = 
	{
		locked = false,
		hidden = false,
	},
	ZonePOPWnd = 
	{
		locked = false,
		hidden = false,
	},
	Options = 
	{
		snapDistance = 20,
		snapWindowsEnabled = true,
	},
	EA_ChatWindowGroup1 = 
	{
		locked = false,
		hidden = false,
	},
	MyHP = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_OpenPartyFlyOutAnchor = 
	{
		locked = false,
		hidden = true,
	},
	LayerTimerWindow = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window785 = 
	{
		locked = false,
		hidden = false,
	},
	DeathblowWin1Window = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFT2 = 
	{
		locked = false,
		hidden = false,
	},
	["DAoCBuff_Slows on me"] = 
	{
		locked = false,
		hidden = false,
	},
	PlayerHP = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_GlyphTracker = 
	{
		locked = false,
		hidden = false,
	},
	EA_ChatWindowGroup5 = 
	{
		locked = false,
		hidden = false,
	},
	DetauntHelper_MonitorAnchorFrame = 
	{
		locked = false,
		hidden = false,
	},
	EA_MoraleBar = 
	{
		locked = false,
		hidden = true,
	},
	MoraleWindow1 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiMyAP = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup4Window = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window1160 = 
	{
		locked = false,
		hidden = false,
	},
	StatDollWnd = 
	{
		locked = false,
		hidden = false,
	},
	MoraleWindow4 = 
	{
		locked = false,
		hidden = false,
	},
	ActionBarLockToggler = 
	{
		locked = false,
		hidden = true,
	},
	EA_Window_ZoneControl = 
	{
		locked = false,
		hidden = true,
	},
	BuffHeadAdvancedContainerLayoutEditorWindow0 = 
	{
		locked = false,
		hidden = false,
	},
	EA_StanceBar = 
	{
		locked = false,
		hidden = true,
	},
	PurePlayerUnitFrame = 
	{
		locked = false,
		hidden = false,
	},
	PureGroupLayout = 
	{
		locked = false,
		hidden = false,
	},
	RetAlert_Main = 
	{
		locked = false,
		hidden = false,
	},
	EA_AssistWindow = 
	{
		locked = false,
		hidden = true,
	},
	PagerAnchor = 
	{
		locked = false,
		hidden = false,
	},
	Xp = 
	{
		locked = false,
		hidden = false,
	},
	RF = 
	{
		locked = false,
		hidden = false,
	},
	BashHPbar = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiCastbar = 
	{
		locked = false,
		hidden = false,
	},
	SquaredAnchor = 
	{
		locked = false,
		hidden = false,
	},
	RoR_RankedLeaderboard = 
	{
		locked = false,
		hidden = false,
	},
	SNT_BUTTONS_FLASH_ANCHOR = 
	{
		locked = false,
		hidden = true,
	},
	EnemyCombatLogTargetDefeseWindow = 
	{
		locked = false,
		hidden = true,
	},
	FiskgjuseUiHTOHP = 
	{
		locked = false,
		hidden = false,
	},
	PureTargetUnitFrameHostile = 
	{
		locked = false,
		hidden = false,
	},
	SquaredIcon = 
	{
		locked = false,
		hidden = false,
	},
	BattlegroupHUDGroup4LayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	["DAoCBuff_My Detaunt"] = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_PublicQuestTracker = 
	{
		locked = false,
		hidden = false,
	},
	RpBarWindow = 
	{
		locked = false,
		hidden = true,
	},
	EnemyKillSpamDialog = 
	{
		locked = false,
		hidden = false,
	},
	x = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFriendlyOverhead = 
	{
		locked = false,
		hidden = false,
	},
	EA_TacticsEditor = 
	{
		locked = false,
		hidden = false,
	},
	TargetHostile = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup1Window = 
	{
		locked = false,
		hidden = true,
	},
	EA_ChatWindowGroup7 = 
	{
		locked = false,
		hidden = false,
	},
	PlayerCareer = 
	{
		locked = false,
		hidden = false,
	},
	EnemyCombatLogTargetDefeseTotalWindow = 
	{
		locked = false,
		hidden = true,
	},
	FiskgjuseUiGI6 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window804 = 
	{
		locked = false,
		hidden = false,
	},
	hostileHP_overHead = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_RvRTracker = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_new_Frame1 = 
	{
		locked = false,
		hidden = false,
	},
	fiendlyHP = 
	{
		locked = false,
		hidden = false,
	},
	EnemyMarksWindow = 
	{
		locked = false,
		hidden = false,
	},
	RoR_RankedLeaderboard_Toggler = 
	{
		locked = false,
		hidden = false,
	},
	GroupMember5 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window802 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiCareer = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup6Window = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_heads = 
	{
		locked = false,
		hidden = false,
	},
	cast2 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1148 = 
	{
		locked = false,
		hidden = false,
	},
	EA_CareerResourceWindowActionBar = 
	{
		locked = false,
		hidden = false,
	},
	sdg = 
	{
		locked = false,
		hidden = false,
	},
	FHP = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window792 = 
	{
		locked = false,
		hidden = false,
	},
	SwiftAssist = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiGI2 = 
	{
		locked = false,
		hidden = false,
	},
	MiracleGrow = 
	{
		locked = false,
		hidden = false,
	},
	MenuBarWindow = 
	{
		locked = false,
		hidden = true,
	},
	GroupMember3 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Self_AddInfo = 
	{
		locked = false,
		hidden = false,
	},
	["DAoCBuff_My HoTs"] = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_RRQTracker = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_Immunities = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window9 = 
	{
		locked = false,
		hidden = false,
	},
	EA_ChatWindowGroup2 = 
	{
		locked = false,
		hidden = false,
	},
	TargetFriendly = 
	{
		locked = false,
		hidden = false,
	},
	MouseOverTargetWindow = 
	{
		locked = false,
		hidden = false,
	},
	EA_CareerResourceWindow = 
	{
		locked = false,
		hidden = true,
	},
	MiracleGrow2 = 
	{
		locked = false,
		hidden = false,
	},
	SoloqOverviewWindow = 
	{
		locked = false,
		hidden = false,
	},
	MiracleGrow2Icon = 
	{
		locked = false,
		hidden = false,
	},
	EnemyCombatLogIDSAnchor = 
	{
		locked = false,
		hidden = false,
	},
	TeatherSelfWindow = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window460 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Self_Debuffs = 
	{
		locked = false,
		hidden = false,
	},
	QueueQueuer_GUI_MapButtons = 
	{
		locked = false,
		hidden = false,
	},
	RV_ModManagerToggleButton = 
	{
		locked = false,
		hidden = true,
	},
	TextEntry = 
	{
		locked = false,
		hidden = false,
	},
	EnemyGuardDistanceIndicator = 
	{
		locked = false,
		hidden = false,
	},
	MoraleWindow3 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Self_Buffs = 
	{
		locked = false,
		hidden = false,
	},
	newcareer = 
	{
		locked = false,
		hidden = false,
	},
	RVMOD_PlayerStatus = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFTOHP2 = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_OverheadMap = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window1117 = 
	{
		locked = false,
		hidden = false,
	},
	FriendsKilledBy = 
	{
		locked = false,
		hidden = false,
	},
	GroupMember1 = 
	{
		locked = false,
		hidden = false,
	},
	BattlegroupHUDGroup2LayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_Hostile_Self_Debuff = 
	{
		locked = false,
		hidden = false,
	},
	EnemyUnitFramesAnchor1 = 
	{
		locked = false,
		hidden = false,
	},
	TidyRollAnchor = 
	{
		locked = false,
		hidden = false,
	},
	EnemyUnitFramesAnchor2 = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup2Window = 
	{
		locked = false,
		hidden = true,
	},
	TextEntryAnchor = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiXP = 
	{
		locked = false,
		hidden = false,
	},
	ObjAnchor = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiGI4 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1123 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window790 = 
	{
		locked = false,
		hidden = false,
	},
	BattlegroupHUDGroup1LayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	EA_GrantedAbilities = 
	{
		locked = false,
		hidden = false,
	},
	RVMOD_ManagerToggleButtonWindow = 
	{
		locked = false,
		hidden = true,
	},
	EA_Window_CityTracker = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_GroupBuffs2 = 
	{
		locked = false,
		hidden = false,
	},
	ResToggle = 
	{
		locked = false,
		hidden = true,
	},
	EnemyTimer = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Self_Morales = 
	{
		locked = false,
		hidden = false,
	},
	Castbar = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHP = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_GroupBuffs4 = 
	{
		locked = false,
		hidden = false,
	},
	EA_ChatWindowGroup4 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window845 = 
	{
		locked = false,
		hidden = false,
	},
	Moth = 
	{
		locked = false,
		hidden = false,
	},
	PlayerAP = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar4 = 
	{
		locked = false,
		hidden = false,
	},
	GroupWindow = 
	{
		locked = false,
		hidden = true,
	},
	HarbingerWindow = 
	{
		locked = false,
		hidden = false,
	},
	DetauntHelperAnchorFrame = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window813 = 
	{
		locked = false,
		hidden = false,
	},
	EnemyIcon = 
	{
		locked = false,
		hidden = false,
	},
	MotionWindow = 
	{
		locked = false,
		hidden = false,
	},
	FriendlyTarget = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1124 = 
	{
		locked = false,
		hidden = false,
	},
	Pet = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar1RightCap = 
	{
		locked = false,
		hidden = true,
	},
	newhp = 
	{
		locked = false,
		hidden = false,
	},
	EA_HelpTipsContainerWindow = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window1111 = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup3Window = 
	{
		locked = false,
		hidden = true,
	},
	EA_ChatWindowGroup6 = 
	{
		locked = false,
		hidden = false,
	},
	MiracleGrowLightAnchor = 
	{
		locked = false,
		hidden = false,
	},
	PureTargetHUDHostile = 
	{
		locked = false,
		hidden = false,
	},
	DetauntHelperIcon = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar1LeftCap = 
	{
		locked = false,
		hidden = true,
	},
	DuffTimer_Win3 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiMyHP = 
	{
		locked = false,
		hidden = false,
	},
	scenarioInfo = 
	{
		locked = false,
		hidden = true,
	},
	PurePlayerPetTargetUnitFrame = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHOHP = 
	{
		locked = false,
		hidden = false,
	},
	GroupMember4 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiGI5 = 
	{
		locked = false,
		hidden = false,
	},
	GroupMember2 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Resistbuff = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_FriendlyBuffs = 
	{
		locked = false,
		hidden = false,
	},
	EnemyCombatLogEpsWindow = 
	{
		locked = false,
		hidden = true,
	},
	EnemyTalismanAlerterIndicator = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_FriendlyDebuffs = 
	{
		locked = false,
		hidden = false,
	},
	GuardList_Window0 = 
	{
		locked = false,
		hidden = false,
	},
	PureTargetUnitFrameFriendly = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1166 = 
	{
		locked = false,
		hidden = false,
	},
	MotionIcon = 
	{
		locked = false,
		hidden = true,
	},
	NerfedIcon = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_HostileDebuffs = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1142 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_GroupBuffs3 = 
	{
		locked = false,
		hidden = false,
	},
	ObsidianCastbar = 
	{
		locked = false,
		hidden = false,
	},
	FiskCastbar = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Hostile_All = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_GroupBuffs0 = 
	{
		locked = false,
		hidden = false,
	},
	FloatingScenarioGroup5Window = 
	{
		locked = false,
		hidden = true,
	},
	DAoCBuff_Self_All = 
	{
		locked = false,
		hidden = false,
	},
	MoraleWindow2 = 
	{
		locked = false,
		hidden = false,
	},
	CMapWindow = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar1 = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window816 = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_QuestTracker = 
	{
		locked = false,
		hidden = false,
	},
	WarbuilderMainWindow = 
	{
		locked = false,
		hidden = false,
	},
	BattlegroupHUDGroup3LayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	Bashhp = 
	{
		locked = false,
		hidden = false,
	},
	StatdollWnd = 
	{
		locked = false,
		hidden = false,
	},
	WCDBAnchorFrame = 
	{
		locked = false,
		hidden = false,
	},
	ChannelAlertWindow = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Friendly_Self_Buff = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Friendly_All = 
	{
		locked = false,
		hidden = false,
	},
	PlayerPet = 
	{
		locked = false,
		hidden = false,
	},
	SecondaryTargetLayoutWindow = 
	{
		locked = false,
		hidden = true,
	},
	["SOR.Button"] = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiHostileOverhead = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Hostile_AddInfo = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiGI3 = 
	{
		locked = false,
		hidden = false,
	},
	color = 
	{
		locked = false,
		hidden = false,
	},
	rroftarget = 
	{
		locked = false,
		hidden = false,
	},
	["DAoCBuff_Hostile Debuffs"] = 
	{
		locked = false,
		hidden = false,
	},
	RoR_SoR_Button = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1105 = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_QuestTrackerNub = 
	{
		locked = false,
		hidden = false,
	},
	GuardRange_Window0 = 
	{
		locked = false,
		hidden = false,
	},
	EnemyStopwatch = 
	{
		locked = false,
		hidden = true,
	},
	RoR_Window_ScenarioSurrender = 
	{
		locked = false,
		hidden = false,
	},
	PlayerWindow = 
	{
		locked = false,
		hidden = true,
	},
	EA_Window_KeepObjectiveTracker = 
	{
		locked = false,
		hidden = true,
	},
	MyAP = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window808 = 
	{
		locked = false,
		hidden = false,
	},
	EnemyTarget = 
	{
		locked = false,
		hidden = true,
	},
	EA_ActionBar3 = 
	{
		locked = false,
		hidden = false,
	},
	DuffTimer_Win1 = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Friendly_Morales = 
	{
		locked = false,
		hidden = false,
	},
	PurePlayerPetUnitFrame = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_CampaignMap = 
	{
		locked = false,
		hidden = true,
	},
	HostileHP = 
	{
		locked = false,
		hidden = false,
	},
	GuardLineSelfWindow = 
	{
		locked = false,
		hidden = false,
	},
	PureTargetHUDFriendly = 
	{
		locked = false,
		hidden = false,
	},
	warwhispererMain = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_GroupBuffs1 = 
	{
		locked = false,
		hidden = false,
	},
	EA_ActionBar5 = 
	{
		locked = false,
		hidden = false,
	},
	DuffTimer_Win2 = 
	{
		locked = false,
		hidden = false,
	},
	EA_ChatWindowGroup3 = 
	{
		locked = false,
		hidden = false,
	},
	RealmAnchor = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Friendly_AddInfo = 
	{
		locked = false,
		hidden = false,
	},
	EnemyKilledBy = 
	{
		locked = false,
		hidden = true,
	},
	LIBGUI_Window1154 = 
	{
		locked = false,
		hidden = false,
	},
	EnemyRCWindow = 
	{
		locked = false,
		hidden = false,
	},
	PureGroupPetLayout = 
	{
		locked = false,
		hidden = false,
	},
	XpBarWindow = 
	{
		locked = false,
		hidden = true,
	},
	RoR_SoR_Main_Window = 
	{
		locked = false,
		hidden = false,
	},
	MouseoverTarget = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window1130 = 
	{
		locked = false,
		hidden = false,
	},
	EnemyPlayerKDR = 
	{
		locked = false,
		hidden = true,
	},
	["Career resources"] = 
	{
		locked = false,
		hidden = false,
	},
	["Info panel"] = 
	{
		locked = false,
		hidden = false,
	},
	newap = 
	{
		locked = false,
		hidden = false,
	},
	DAoCBuff_Slows = 
	{
		locked = false,
		hidden = false,
	},
	Minmap = 
	{
		locked = false,
		hidden = false,
	},
	PlayerPetTarget = 
	{
		locked = false,
		hidden = false,
	},
	Castbar2 = 
	{
		locked = false,
		hidden = false,
	},
	EA_Window_WinOMeter = 
	{
		locked = false,
		hidden = false,
	},
	LIBGUI_Window14 = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFTOHP = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFT = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiGI1 = 
	{
		locked = false,
		hidden = false,
	},
	WCDPLayoutFrame = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiAP = 
	{
		locked = false,
		hidden = false,
	},
	FiskgjuseUiFOHP = 
	{
		locked = false,
		hidden = false,
	},
}



